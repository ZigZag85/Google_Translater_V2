			...
			try:
					for i in list:
						...
						if list[i] == translation.dest:
							to = i
					await message.reply_text(f"Tərcümə edilmişdir  **{fromt.capitalize()}** To **{to.capitalize()}**\n\n```{translation.text}```\n\n join @lntechnical")
			except Exception as e:
					await message.reply_text(f"Tərcümə edilmişdir  **{translation.src}** To **{translation.dest}**\n\n```{translation.text}```\n\n join @lntechnical")
	else:
...
async def translate_text(bot,update):
	...
	[InlineKeyboardButton(" Next --->",callback_data = "page2")
      ...
      cb_data = update.data
      if cb_data== "page2":
      	await update.message.edit("Dil seçin 👇",reply_markup = keybord2)
      elif cb_data == "page1":
      	await update.message.edit("Dil seçin  👇",reply_markup =keybord1)
      elif cb_data =="page3":
      	await update.message.edit("Dil seçin  👇",reply_markup =keybord3)
      elif cb_data == "page4":
      	await update.message.edit("Dil seçin  👇",reply_markup =keybord4)
      elif cb_data =="page5":
      	await update.message.edit("Dil seçin  👇",reply_markup =keybord5)
      elif cb_data =="page6":
      	await update.message.edit("Dil seçin  👇",reply_markup =keybord6)
      else :
      		try:
      			translator = Translator()
      		...
      		try:
      			for i in list:
      				...
      				if list[i] == translation.dest:
      					to = i 
      			await update.message.edit(f"Tərcümə edilmişdir  **{fromt.capitalize()}** To **{to.capitalize()}**\n\n```{translation.text}```\n\n join @lntechnical")
      		except Exception as e:
      			await update.message.edit(f"Tərcümə edilmişdir  **{translation.src}** To **{translation.dest}**\n\n```{translation.text}```\n\n join @lntechnical")
